// src/features/explore_feed/RecentCollections.js
...
import { getRecentPublicCollections, getUserProfile } from '../../firebase/firestore';
...
const RecentCollections = () => {
  ...
  const fetchRecent = useCallback(async () => {
    ...
    setError(null);
    try {
      const { collections: fetched } = await getRecentPublicCollections(20);
  ...
  if (!collectionsWithUsernames || collectionsWithUsernames.length === 0) {
    return (
      <View style={styles.centered}>
        <Ionicons name="time-outline" size={60} color={colors.textLight} style={{marginBottom: 10}}/>
        <Text style={styles.emptyText}>Henüz herkese açık koleksiyon yok.</Text>
      </View>
    );
  }
